...
CREATE TABLE `notifications` (
  ...
  `type` enum('appointment_request','appointment_accepted','appointment_rejected','reminder') DEFAULT NULL,
...
CREATE TABLE `rooms` (
  ...
  `status` enum('active','expired','waiting') DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
...
ALTER TABLE `follows`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_follow` (`patient_id`,`doctor_id`);
...
ALTER TABLE `appointments`
  ...
  ADD CONSTRAINT `appointments_ibfk_3` FOREIGN KEY (`slot_id`) REFERENCES `availability` (`id`),
